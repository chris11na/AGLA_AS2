...
int main() {
    ...
    if (n == 1){
       ...
       ColumnVector vector1(m);
        ...
        double A = 0;
        double B = 10;
        fprintf(pipe, "plot [%lf:%lf]  %lf*x + %lf, '-' using 1:2 with points\n", A, B, x.get(1,0), x.get(0,0));
        double step = (B - A) / 9;
        for (double k = A; k <= B; k += step) {
            double y = x.get(1,0)*k + x.get(0,0);
            fprintf(pipe, "%f\t%f\n", k, y);
        }
    ...
    if (n == 2){
        ...
        double A = 0;
        double B = 10;
        fprintf(pipe, "plot [%lf:%lf] %lf*x**2 + %lf*x + %lf, '-' using 1:2 with points\n", A, B, x.get(2,0), x.get(1,0), x.get(0,0));

        double step = (B - A) / 9;
        for (double k = A; k <= B; k += step) {
            double y = x.get(2,0)*k*k + x.get(1,0)*k + x.get(0,0);
            fprintf(pipe, "%f\t%f\n", k, y);
        }
